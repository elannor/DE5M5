...
def library_cleaning_data(input_path: str, output_path: str=None) -> str:
    ...
    df_metrics.to_sql("library_metrics", con=engine, if_exists="append", index=False)
    df_metrics.to_csv("library_metrics.csv", mode='a', header=not Path("library_metrics.csv").exists(), index=False)
